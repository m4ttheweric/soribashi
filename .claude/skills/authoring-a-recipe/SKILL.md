...
This is a DIFFERENT mechanism from `installNoTransitionStyle()` in `packages/ui/src/a11y/matrix-harness.tsx`. That helper is scoped to the a11y contrast-matrix tier only (imported by `contrast-matrix.test.tsx`, never by a `.visual.test.tsx` file), and its installed rule covers `transition: none !important` only, no `animation` override, because no `SMALL_COVERAGE`/full-grid contrast cell before Skeleton ever needed one. A future colour-bearing recipe with its own perpetual `animation` (not just a `transition`) that needs contrast-matrix coverage would need the matrix harness extended the same way Skeleton's own visual test was; that extension does not exist yet.
