...
`Field` (`packages/ui/src/recipes/Field/Field.tsx`) is two things at once: a public compound (`Field.Root`/`Field.Label`/`Field.Description`/`Field.Error`, `defineCompound` over `@base-ui/react/field`, four slots that are simultaneously real Base UI parts and real style slots) a consumer can hand-compose directly, AND the internal substrate every form control with label/description/error convenience props (TextInput, Textarea, Switch, RadioGroup, Checkbox) renders for itself, unexported and invisible to their own consumers. TextInput.tsx is the reference implementation; the others copy its render shape (Checkbox with one locked exception, below).

**The conditional `Field.Root` rule.** A control with `label`/`description`/`error` convenience props does NOT always render `Field.Root`. It renders ONLY the bare control (no wrapper at all) when none of the three props are present, and renders `Field.Root` wrapping `Field.Label`/the control/`Field.Description`/`Field.Error` (in that order) only when at least one is. This is a real branch in every consuming control's render body (`const hasAnatomy = label != null || description != null || error != null;`), not a cosmetic default.

**Checkbox's label-only exception (locked compatibility decision, 2026-08-06 debt sweep).** Checkbox predates the contract and keeps its original `label` behaviour: `label` ALONE does not trigger anatomy mode -- it renders the native label-wraps-control containment `<label>` (real association, no `Field.Root` in the tree), pixel-identical to every pre-migration call site. Only `description`/`error` presence switches Checkbox into `Field.Root` mode (`hasFieldAnatomy = description != null || error != null`), and only those two arm the nested-hand-composed-Field dev warning; a label-only Checkbox inside a hand-composed `Field.Root` therefore renders two competing labels with no warning (documented on `CheckboxProps.label`). Two further Checkbox-specific notes: its anatomy row renders the CONTROL before `Field.Label` (box-left is the checkbox convention; deliberately diverges from Switch's label-first row), and its bare-mode layout CSS is scoped as `label.root` (type+class) because Checkbox and Field both declare a same-specificity `.root` rule in the same layer and cross-file order is not deterministic under Vite's CSS-module concatenation -- a new anatomy consumer that styles its own `root` slot must scope its bare-mode-only rules the same way. Known consequence of bare mode: with no `root` element, `getStyles('root')` is never called, so `data-size`/`data-intent` never land anywhere in bare mode either; sizing and colour still ride the control's own dimension/intent vars (`--sb-textinput-h`, `--sb-switch-checked-bg`, ...) regardless of whether the data attribute is present.
...
## 23. The manager-driven compound (global Provider + imperative queue): Toast

Toast is the first recipe with a global `Provider` and an imperative manager (`useToast()`) rather than a trigger that opens one popup. A future recipe of the same shape (a notification/snackbar system, an imperative confirm queue) follows this pattern; nothing else in the package does.

**The Provider MUST be `defineCompound`'s `parts.root`.** A compound part's `getStyles(...)` needs a non-null compound context, which exists only inside the compound root's subtree. The consumer renders `<Toast.Provider>{app}<Toast.Viewport/></Toast.Provider>`, so if the Provider were NOT the root, the `Viewport` (a sibling/descendant of the app, outside any compound root) would hit `rawCtx === null` and `getStyles('viewport')` throws (verified in `define-compound.tsx`). Make the Provider the root part; it wraps the Base UI provider and has no style slot of its own (like Dialog's root). Public surface stays `Toast.Provider` (the root used bare, via `Object.assign(Compound, { Provider: Compound })`) plus `Toast.Viewport`.

**Slot-name collision.** `parts.root` is the Provider (no DOM). The per-instance surface is therefore a DIFFERENT slot key (Toast uses `toast`, not `root`), so name it accordingly and keep `<Name>Root` (Base UI's per-instance element) out of the public surface.

**Parts vs. style slots.** Declare only `root` (Provider) and `viewport` as `defineCompound` `parts`; the per-instance anatomy (`toast`/`title`/`description`/`close`/`action`) are **style slots in `slotKeys` but not parts**, rendered inside the Viewport part's queue map and addressed with `getStyles({ part: 'toast' })`. Declaring the per-instance surface as a part would surface an odd `Toast.Toast` public member. The base library's own Viewport is not batteries-included (it renders only children plus focus guards plus a hidden live region, and does not self-portal), so the recipe's Viewport part wraps `Portal` + the base Viewport and owns the `manager.toasts.map(...)` itself.

**Dynamic per-instance intent.** Intent is set imperatively (`add({ intent })` maps to the base library's per-toast `type`), NOT as a static vocabulary-axis prop, so `getStyles`'s stamped `data-intent` and the compound `vars` resolver (which runs once) cannot supply it. Compute colour per instance inside the Viewport map, gated on the value being present: `const vars = t.type ? (autoVars(theme, 'Toast', { intent: t.type, variant: ctx.variant }, true).root ?? {}) : {};` where `theme = useTheme()` (hooks are legal in a part render body, §10) and `ctx.variant` is the static axis. Spread `vars` as inline style; hand-stamp `data-intent={t.type}`/`data-variant={ctx.variant}` (§7). Gate on `t.type`: calling `autoVars` with no intent both returns `{}` AND dev-warns ("declares variants but has no intent default"), so a no-intent instance keeps a neutral CSS fallback (`var(--toast-bg, var(--surface-raised))`) with no warning. The imperative options type keeps `intent?: string` (loose), NOT the theme's intent union: the vendored recipe stands alone and has no opinion on vocabulary values (invariant 2).

**Testing traps this pattern hits (all bit during Toast's build):**

- **Contrast uses `SMALL_COVERAGE`, not `describeColourGrid`.** The grid queries its own `container` by `data-testid` and requires the measured element inside that container; a manager-driven surface **portals to `<body>`**, outside it. `SMALL_COVERAGE` queries `document` and waits via `vi.waitUntil`, so it reaches the portal. Render one Provider tree per variant, fire one `timeout: 0` instance per intent from a mount effect (state at mount only, §16), tag the surface slot per variant via `classNames`, and select each cell by `.<class>[data-intent="..."][data-variant="..."]` so every cell resolves to exactly one element (`resolveUniqueTarget` throws on multi-match).
- **Reskin fixture** forwards `scopeEl` into the Viewport's portal `container` (§15) and fires one `timeout: 0` instance at mount, tagging the surface slot `reskin-target`.
- **Auto-dismiss test uses REAL timers plus `vi.waitFor`/retrying `expect.element(...).not.toBeInTheDocument()`**, not fake timers: `add({ timeout: 150 })`, then poll for removal. This sidesteps the fake-timer + `act()` trap in §10 entirely.
- **First import of the base library's new subpath** (Toast was the first `@base-ui/react/toast`) hits the stale-Vite-dep-cache false negative (§10: `Cannot read properties of null (reading 'useRef')` deep in the base library). `rm -rf node_modules/.vite packages/ui/node_modules/.vite` and re-run.
- **No `--spacing-2xs` and no `--z-index-*` tokens exist** in this theme. `token-existence.test.ts` fails any `var(--token)` no theme emits: the smallest spacing is `--spacing-xs`, and stacking uses a raw unitless `z-index` (Select's `.popup` does the same), which the value scanner allows.
