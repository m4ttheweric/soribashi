...
- **Popover's popup has no explicit font.** `Popover.module.css`'s `.popup` rule declares no `font-family`/`font-size`, so it falls back to serif in its baselines; the fix-wave that closed this same gap for Select deliberately scoped Popover out (`41ca145`), since Popover's baselines were not part of that wave's committed set. Fixing it needs its own baseline regeneration.
- **No regression test pins the Tabs raw-attribute strip.** The root part destructures `variant`/`size`/`intent` out before spreading onto the DOM element (fixed in slice 3, `22d3c4b`), but nothing asserts the raw `variant` HTML attribute stays absent going forward.
- **No test pins neutral-wash-vs-canvas distinctness.** Nothing fails today if the ghost hover wash regresses to being identical to the canvas; that is a distinctness bug, not a contrast bug, so the contrast matrix cannot catch it.
