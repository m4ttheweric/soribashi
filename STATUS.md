...
- All six CI jobs (`checks`, `codegen-drift`, `registry-smoke`, `browser`, `visual`) are live and un-gated; none are skipped or marked allow-failure.
...
- **Dark-scheme contrast follow-up.** The known, un-allowlisted dark-scheme contrast gaps noted above need new dark-specific text colours per intent, a design decision, not a value-level token nudge, and were deliberately left open rather than allowlisted away.
