...
> **Current as of 2026-07-26.** Slice 3 (below) is the current top record: five new recipes (Alert, Badge, Checkbox, Tabs, Select), completing all four authoring categories in `@soribashi/ui` for the first time. It sits above the slice 2 record, the slice 1b record, the recipe-pilot record (2026-07-01), and the v1 Mantine-adaptation foundation record (2026-04-25), all kept unchanged further down this file.
...
Branch `feat/slice-3-components`, from `main` @ `0bc37ac`, 20 commits, 10 of a planned 12 numbered tasks executed in strict order (9 recipe/workshop tasks plus this closing docs task); task 11 (committing Linux visual baselines) and task 12 (whole-branch review) are still ahead and not part of this task's scope.

### What landed

- **Two framework prerequisites, resolved before any recipe landed.** `defineCompound` gained a runtime slot-key source distinct from its parts map (`slotKeys`, backed by a `const SLOT_KEYS = [...] as const` array a compound declares itself), closing the gap flagged in slice 1b's future list ("compound slot metadata needs a runtime source on `defineCompound`"). `bun run smoke:registry`'s scratch `pkg.dependencies` now derives from `item.dependencies` instead of hardcoding `@soribashi/core`, closing the gap flagged in slice 2's future list, and further work here (rewriting vendored `workspace:*` deps to `file:`) made the real `shadcn` CLI path work for a Base UI-backed item for the first time.
- **Alert** (category 1, `defineComponent`, 5 style slots: `root`/`icon`/`title`/`body`/`close`) and **Badge** (category 1, `defineComponent`) are the package's first multi-slot recipes that are not compounds; see the authoring skill's new "multi-slot single-component pattern" section. Both declare their own three-value variant subset (`filled`/`outline`/`subtle`), not the theme's five, since `ghost`/`link` have no sane rendering for either. Both ride the theme's `primary`/`filled` intent-resolver pairing, which measures 4.5617:1 in both schemes, 0.0617 above the 4.5 AA floor; see "deliberately future" below.
...
- `bun run test:visual` and `bun run smoke:registry`: **not run in this task.** Linux visual baselines for the five new recipes are the next task's job (committing baselines captured before the Checkbox indeterminate fix would have frozen the wrong glyph, which is why that fix had to land in this task rather than after); `smoke:registry` is network-dependent and slow, and nothing in this task touches the registry beyond regenerating it via `generate:ui` for the Checkbox CSS/TSX content change. Reporting this rather than implying a clean sweep across gates that were not actually run.
...
- **An open flake.** One full-suite axe failure appeared during Task 8 (Select) and did not reproduce across three subsequent full runs. A reviewer hunted the Select diff for a leak mechanism (portal left mounted, style element not removed, body class not cleaned, unawaited unmount) matching Popover's own existing patterns, and found none. It is unexplained, not dismissed; nothing here should be read as "investigated and ruled out."
- **A lead worth checking, not yet investigated.** `Button.tsx` does not supply explicit generic params despite declaring `vocabularyAxes`, while Text, Container, Alert, and Badge do (the skill's own "generic-params trap"). Separately, `defineComponent`'s theme-independent export types `size`/`intent` as bare `string`, narrowed only through a `makeBuilders<TTheme>()` wrapper that no recipe in this package currently uses (Alert, Badge, and Checkbox all share this). If both hold, the vocabulary narrowing this package advertises reaching a recipe's public prop types may not actually be reaching any of them. Recorded as an open question raised during review, not a conclusion: nobody has yet written the throwaway type-level probe that would settle it.
