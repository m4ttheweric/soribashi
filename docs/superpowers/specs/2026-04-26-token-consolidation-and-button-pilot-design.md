...
**Target host (read-only reference):** `<host-library-path>` (the host library)
...
The host library consumes a local UI layer in `the host component library/host/` — 28 component groups built on Radix primitives + Tailwind, imported by 115 files across the host library. The framework story underneath that layer is fragmented:

- **Three parallel color-token systems coexist** in `host-styles.css`:
...
- Token consolidation pass: inventory + classification of every token in the host library's `tailwind.config.js` and `host-styles.css`; expression of the kept tokens as a soribashi theme; deprecation list with rationale; open design questions surfaced for human design review.
- A pilot host app: `apps/pilot/` — a new Vite + React + TS app inside soribashi alongside `apps/playground`, with its own `tailwind.config.js`, theme, and a representative-host-screen replica for visual review.
- Button conversion: `apps/pilot/src/recipes/Button/` — base Button only, authored with `defineComponent`, consuming the consolidated theme. Full variant × intent × size × state matrix. Polymorphism (`as="a"`).
...
- Wave 1 partial playbook: `docs/superpowers/specs/2026-04-26-recipe-conversion-playbook.md`.

### Out of scope (explicit)

- Touching `the host monorepo` in any way. No code changes there. No package installs there.
- Editing the host library's existing `host/` in place.
- Pilots for Tooltip, Tabs, Select — explicitly Wave 2+, sized but not designed in this spec.
- The other 24 host component groups.
...
- Migrating the host library's existing 115 importers from fragmented tokens to the consolidated vocabulary — that is the eventual integration project, scoped (not designed) by the playbook.
...
**Wave 1 history:** an earlier draft of this spec framed Option A (soribashi owns the entire Tailwind config) as the north star and Option C (composition) as a working state. That framing was decided in a single 33-minute exchange on 2026-04-26 with thin justification — the argument for A reduced to "soribashi authoring host as a library implies soribashi can own the Tailwind layer," which is tautological rather than substantive. After the Wave 1 pilot landed without surfacing any friction with composition, the decision was revisited on 2026-05-02 and reversed: **Option C is the integration model, period.** No "C → A bridge" — just C.

**What still applies from the original Wave-1 framing:**

- **Configurable dark-mode selector** — was originally listed as an Option-A feature. **Already implemented and tested** (verified 2026-05-02 — the original "codegen hardcodes `.dark`" framing was incorrect; the configurability has been wired in since well before Wave 1). `ThemeDefinition.darkMode.selector` defaults to `.dark` and flows through `createTheme` → `emitCss`; consumers like the host library can pass `darkMode: { selector: '.dark .app-scope' }` directly to `createTheme`. See `packages/codegen/src/emit-css.ts:36,59` for the emit + two existing tests at `packages/codegen/test/emit-css.test.ts:120` (the host library's exact `.dark .app-scope` selector) and `packages/codegen/test/emit-css-emitter-logic.test.ts:324` (`[data-theme="dark"]`). No codegen change needed for the integration project.
...
`apps/pilot/` — new Vite + React + TS app inside the soribashi monorepo, alongside `apps/playground`. Standalone — does not consume `apps/playground`.
...
│   ├── reference/
│   │   └── original-button/  # vendored snapshot of the host library's current Button (for side-by-side visual review only — see § 7.1 step 3)
│   ├── recipes/
│   │   └── Button/
│   │       ├── Button.tsx
│   │       ├── Button.test.tsx
...
│       ├── ScreenReplica.tsx   # representative the host library screen replica for visual review
...
module.exports = {
  ...
  corePlugins: {
    preflight: false, // mirrors the host library's setting so utility specificity matches
  },
  plugins: [
    ...(generated.plugins ?? []),
...
3. **Express the signal as a soribashi theme** in `apps/pilot/src/theme/index.ts`:
   ...
   - **Dark variants** for everything emittable in soribashi's dark-token model. Wave 1 default selector is `.dark`; the host library's scoped `.dark .app-scope` is a North-star-A item, not a Wave 1 problem.
...
6. **Render the screen replica.** `pages/ScreenReplica.tsx` is a static markup-only replica of one representative the host library screen (suggested: an Auto Statements Overview section — pick one with cards, headings, body text, badges-ish chips, divider lines). Uses Tailwind classes consuming the consolidated tokens. **No live host or recipe components yet** — this page exists to validate that the consolidated tokens reach the DOM correctly.
7. **Visual review.** Compare the replica side-by-side with a screenshot of the same the host library screen. **Intent parity, not pixel parity.** Expected drift wherever consolidation deliberately changed something (the new primary ramp, dropped duplicates resolved against the canonical name, etc.). Drift that is *not* expected goes in the journal as a finding.

### 6.2 Deliverables

- `apps/pilot/` host app skeleton — buildable, dev server runs.
- `apps/pilot/src/theme/index.ts` — the consolidated soribashi theme.
...
- **Surfaces:** Collapse `card` + `popover` into a single `surface` token, or keep two? the host library currently has both with the same value; consolidating to one is cleaner; keeping two preserves the option to differentiate elevation later.
...
- **Dark scoping:** Wave 1 emits dark vars under `.dark`. the host library's production scope is `.dark .app-scope`. The integration project (not Wave 1) reconciles. Documented in the playbook.
...
1. **Inventory the current Button surface.** Read `apps/adjuster/src/components/the host component library/Buttons/Button.tsx` (and `IconButton.tsx`, `ButtonDropdown.tsx` for context only — they are not built). Enumerate the variant, intent, size, and state shape currently exposed. Document any inconsistencies (e.g., a "danger" variant that's actually intent-shaped, or a variant that overlaps with an intent).
2. **Author the recipe.** `apps/pilot/src/recipes/Button/Button.tsx`, using `defineComponent` from `@soribashi/factory`. Consume the consolidated theme directly — never reference the legacy fragmented tokens. Cover the full matrix:
...
3. **Demo page.** `pages/ButtonMatrix.tsx` renders the full matrix in both light and dark, side-by-side with a vendored snapshot of the host library's current `host/Button` for visual reference. The reference snapshot is **copied** (not symlinked, not installed as a dep) into `src/reference/original-button/` from the the host monorepo path on a one-time basis as part of Phase 1 setup, with a comment header naming the source path and snapshot date. The vendored copy is read-only reference material — never edited, never imported by the recipe code, never shipped beyond the pilot app. It exists solely so the side-by-side review can happen inside one running app. Include a "consolidation notes" callout per variant where the new shape differs from the legacy shape.
4. **Tests.**
...
- `apps/pilot/src/recipes/Button/{Button.tsx, Button.test.tsx, Button.stories.tsx}`.
- `apps/pilot/src/pages/ButtonMatrix.tsx` — demo page rendered in the pilot app.
- Playwright parity tests in `apps/pilot/tests/`.
...
`docs/superpowers/specs/2026-04-26-recipe-conversion-playbook.md` containing:

1. **Token consolidation methodology.** Transferable to any fragmented design system, not specific to the host library. The inventory → classify → express → codegen → review → decide loop, with the rationale for each step. Half the value of the engagement.
...
4. **Legacy-token migration strategy stub.** What an integration project would need to do to migrate the host library's 115 importers from fragmented tokens to the consolidated vocabulary. Sized (S/M/L estimates per phase) but not designed. Becomes the input to the integration project's own brainstorm.
...
| Soribashi's intent resolver isn't expressive enough for the host library's semantic surface tokens. | Medium | Logged as a gap; pilot extends the resolver inside the pilot work if the gap is small and well-shaped, otherwise documents the gap and uses a hand-written shim locally so Phase 1 continues. |
...
- An evidence-backed answer to: *Can soribashi consolidate the host library's tokens, and is the authoring story real for at least the pure-primitive category?*
...
- **Wave 4 — Select (form control).** The boss. Heaviest anatomy in host; field composition (label/help/error slots); controlled state; keyboard a11y; trigger-vs-content surface tokens; option rendering.
...
- Target host: `<host-library-path>/`
  - `tailwind.config.js` — current Tailwind config (mixed token taxonomies)
  - `host-styles.css` — current CSS var declarations (three parallel color systems)
  - `Buttons/Button.tsx` — legacy Button surface to inventory
  - `HOST_ARCHITECTURE.md` — the host library architecture doc
- Soribashi prior art:
