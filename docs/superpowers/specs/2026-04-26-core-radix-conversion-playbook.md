...
> **Codegen pattern (Gap 1 — RESOLVED post-Wave-1):** the codegen emits **two** CSS vars per color: the canonical wrapped var (`--color-primary-500: hsl(221.2 83.2% 53.3%);`) for direct CSS use, and a `-hsl` companion (`--color-primary-500-hsl: 221.2 83.2% 53.3%;`) for Tailwind's `<alpha-value>` pattern and any consumer needing alpha (`hsl(var(--color-X-Y-hsl) / 0.5)`). The Tailwind config wires the `-hsl` companion automatically, so `bg-primary-500/50` works out of the box. **Recipe authors:** prefer the canonical var for opaque chrome; reach for the `-hsl` companion (or the Tailwind alpha utility) for translucent surfaces, focus rings, and hover tints. Wave 1's Button.css predates the fix and uses only the canonical pattern — that's still correct, just no longer the only option. (Conversion journal § 4 Gap 1 — RESOLVED.)
...
|---|---|---|---|---|
| 1 | Codegen emits `hsl(...)`-wrapped var values; bare-HSL emit needed for Tailwind `<alpha-value>` and `hsl(var(--x))` consumer patterns | ~~important~~ → **resolved** | Consolidation journal § 6 (third bullet) AND conversion journal § 4 Gap 1 — same root cause, surfaced first in Phase 0 Task 0.8 (TokenReview), recurred as a tax in Phase 1 Tasks 1.5 / 1.6 / 1.8 | **RESOLVED post-Wave-1.** Implemented as a **dual-emit** pattern (rather than the bare-only switch originally proposed, which would have broken ~146 existing consumers): codegen now emits both the canonical wrapped var (`--color-primary-500: hsl(...)`) AND a `-hsl` bare-component companion (`--color-primary-500-hsl: ...`). Tailwind config's `<alpha-value>` pattern uses the `-hsl` companion. Zero breaking changes; `bg-primary-500/50` and friends now work. Implementation: `packages/codegen/src/emit-css.ts` (`stripHslWrapper` helper) + `packages/codegen/src/emit-tailwind-v3.ts`. The C → A bridge's gating gap is closed. |
...
- **Gap 1 (codegen `hsl(...)` wrapper) — RESOLVED post-Wave-1.** Was the only Wave 1 gap blocking Option A directly. Tailwind's `<alpha-value>` pattern requires bare HSL components so the utility can splice the alpha in (`bg-primary-500/50` → `hsl(var(--color-X-Y-hsl) / 0.5)`). Resolved via dual-emit: codegen emits both the canonical wrapped var (`--color-X-Y`) for direct CSS use and a `-hsl` bare-component companion for Tailwind alpha. Zero breaking changes to existing consumers; alpha utilities now work. See Gap 1 row above for implementation details.
