...
- **Configurable dark-mode selector — open codegen correctness gap (independent of A vs C).** Today codegen hardcodes `.dark { ... }` in the emitted theme.css. Consumers like CVI need a scoped dark selector (`.dark .claim-view-islands`) because they share a page with non-soribashi UI. The selector is configurable in `ThemeDefinition.darkMode.selector` but isn't read at emit time. Required for the CVI integration project regardless of integration model. Remains an open follow-up.
