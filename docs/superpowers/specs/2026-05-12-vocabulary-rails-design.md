...
2. **`@soribashi/factory` — themed builder return.** `createSoribashiBuilders<TTheme>(theme)` casts the returned `definePolymorphicComponent` to `ThemedDefinePolymorphicComponent<TTheme['vocabulary']>`, whose produced component folds `ThemedVocabularyProps<TVocab, TVocabAxes>` into its **public** props. `ThemedVocabularyProps` injects only the **global** axes (`GlobalVocabularyAxis = 'size' | 'intent'`); `variant` is excluded because it is per-recipe (a global-vocab variant would intersect to `never` against Tooltip's `default|subtle`). The builder implementation is unchanged at runtime — only the declared type is refined, so the cast is sound. `defineComponent`/`defineCompound`/`defineGenericComponent` are returned raw; the pilot's only global-axis consumer is the polymorphic `Button`, and extending the same pattern to the others is mechanical when a consumer appears.

3. **Recipes.** `Button` drops `size`/`intent` from `ButtonOwnProps` (now injected, narrowed); `variant` stays local. This resolves the §17.2 finding: the public component type now *does* surface the injected props. A compile-time test (`Button.test.tsx`, enforced by `bun run typecheck`) asserts `size`/`intent`/`variant` each reject out-of-vocab values via `@ts-expect-error`.
...
**Scope note:** threading covers the global axes through `definePolymorphicComponent` only. Compounds (Tooltip/Tabs) use `variant` exclusively, which is already narrowed recipe-locally, so they need no threading.
