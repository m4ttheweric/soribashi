...
- **OQ-10.** ~~Same per-recipe TProps pattern in `definePolymorphicComponent`.~~ **RESOLVED in-Wave-2 (carry-in cleanup).** Wave 1's Button recipe carried a `props as ButtonOwnProps & { ... [k: string]: unknown }` cast block — laundering the styles-API framework keys (typed against `<any>`) plus adding an index signature so `...rest` could spread on `<Element>`. Fixed by adding `PolymorphicRenderCtx<TOwnProps, TDefaultAs, TSelectors, TVariants>` to the factory: `props` is typed as `TOwnProps & StylesApiProps<TPayload> & ComponentPropsWithoutRef<TDefaultAs> & VariantFields`. Authors annotate the `render` parameter with this type; destructuring is fully typed; `...rest` spread on `<Element>` works without an index-signature cast. Wave 1 Gap 2 stays "documented as convention" for the destructure-the-framework-keys part (composition forwarding rationale), but the cast block is gone. Public API gains `PolymorphicRenderCtx`. Playbook § 2.1 updated.
