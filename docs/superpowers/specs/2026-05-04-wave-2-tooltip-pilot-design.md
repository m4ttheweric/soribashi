...
- **OQ-9.** Per-part prop typing in `defineCompound`. **Surfaced during Wave 2 implementation, deferred.** The factory's part-component generation uses `forwardRef<unknown, any>` — so consumers of a `defineCompound`-generated part don't get typed props on `Tooltip.Provider`, `Tooltip.Trigger`, etc. The Tooltip pilot works around this with a recipe-level type-surface cast at the bottom of `Tooltip.tsx` (~10 lines). The proper fix is to make `defineCompound` generic over per-part prop types so each part config can declare its props (e.g., via `satisfies StandardPartConfig<TooltipProviderProps>`) and the factory threads those types through to both the render's `props` parameter AND the static-property's component type — eliminating both the recipe-internal `as`-casts inside renders AND the recipe-level cast at export. Estimated ~30 lines of TS type-machinery work in `define-compound.tsx`. Land this in Wave 3 when Tabs forces a second compound recipe to write the same workaround twice.
