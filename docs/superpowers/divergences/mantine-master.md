...
- **Soribashi behavior:** `(Wrapped as any).withProps = makeWithProps(Wrapped)` — Mantine passes the WRAPPED component. Each level of wrapping stacks presets incrementally. In a double-wrap chain `A.withProps({x:1}).withProps({x:2})`, the second renders into the first wrapper, which renders into the original; `{x:2}` presets override `{x:1}` presets (but neither overrides explicit instance props). The undefined-filter means only truly undefined instance slots fall through to the next level.
