...
export interface EmitTailwindV3Options {
  /**
   * Whether theme.css emits `--__hsl-color-*` companion vars for HSL tokens.
   * Must match the resolved value used for emitCss so the generated config
   * never references companions that were not emitted. @default true
   */
  emitCompanionHsl?: boolean;
}

/**
 * Emits a Tailwind v3 config snippet that maps token names to CSS variables.
 *
 * HSL color tokens use the `<alpha-value>` pattern (via their `--__hsl-color-*`
 * companion var) so utility classes like `bg-primary-500/50` work correctly.
 * Tokens without a companion (non-HSL values, or emitCompanionHsl=false) fall
 * back to the canonical wrapped var, which does not support alpha utilities.
 */
export function emitTailwindV3(theme: ResolvedTheme, opts: EmitTailwindV3Options = {}): string {
  const companions = opts.emitCompanionHsl !== false;
    ...
    for (const [family, scale] of families) {
      ...
      for (const [shade, value] of shades) {
        if (companions && stripHslWrapper(value) !== null) {
          ...
          // No --__hsl companion exists for this token, so reference the
          // canonical var directly.
          lines.push(
            `        '${shade}': 'var(--color-${family}-${shade})', // alpha utilities (e.g. /50) unsupported: no HSL companion var`,
          );
        }
      }
      lines.push('      },');
