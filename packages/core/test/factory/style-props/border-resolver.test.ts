...
import { borderResolver } from '../../../src/factory/style-props/resolvers/border-resolver.ts';
...
describe('borderResolver', () => {
  ...
  it('"1px solid red" → token-resolved when theme declares a red family', () => {
    ...
    expect(borderResolver('1px solid red', theme)).toBe('0.0625rem solid var(--color-red-500)');
  });
  ...
  it('"1px solid var(--my-color)" → CSS var passes through', () => {
    expect(borderResolver('1px solid var(--my-color)')).toBe('0.0625rem solid var(--my-color)');
  });
