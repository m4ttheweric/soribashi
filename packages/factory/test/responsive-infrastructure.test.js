...
describe('InlineStyles', () => {
    it('renders a style element with base styles', () => {
        const { container } = render(_jsx(InlineStyles, { selector: ".my-class", styles: { padding: '1rem', color: 'red' }, media: {} }));
    ...
    it('renders @media queries for media-specific styles', () => {
        const { container } = render(_jsx(InlineStyles, { selector: ".my-class", styles: {}, media: { '(min-width: 48em)': { padding: '2rem' } } }));
    ...
    it('converts camelCase prop names to kebab-case', () => {
        const { container } = render(_jsx(InlineStyles, { selector: ".x", styles: { marginInlineStart: '1rem', borderRadius: '4px' }, media: {} }));
        const text = container.querySelector('style')?.textContent ?? '';
    ...
    it('preserves CSS custom property names (--my-var)', () => {
        const { container } = render(_jsx(InlineStyles, { selector: ".x", styles: { '--my-var': '1rem' }, media: {} }));
        const text = container.querySelector('style')?.textContent ?? '';
        expect(text).toContain('--my-var: 1rem');
    });
    it('omits undefined and null values', () => {
        const { container } = render(_jsx(InlineStyles, { selector: ".x", styles: { padding: '1rem', margin: undefined, border: null }, media: {} }));
        const text = container.querySelector('style')?.textContent ?? '';
...
//# sourceMappingURL=responsive-infrastructure.test.js.map