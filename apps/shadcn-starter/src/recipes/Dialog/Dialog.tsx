 ...
 *     getStyles({ part: 'overlay' }) / getStyles({ part: 'closeButton' }))
 *     rather than as separate consumer-facing parts.
 ...
 *     footer action).
 *
 * `overlay` and `closeButton` are keys in `classes` but not in `parts`: they
 * are style-addressable via getStyles({ part: 'overlay' | 'closeButton' })
 * without being separate compound parts (overlay has no consumer-facing
 * wrapper; `closeButton` backs the Content-internal X button only). `close`
 * is both a `classes` key and a consumer-facing part (Dialog.Close) -- its
 * styling is corner-position-free interactive-only styling, distinct from
 * `closeButton`'s absolute corner positioning.
 */
...
const classes = {
  ...
  content: [
    'fixed left-1/2 top-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 sm:max-w-lg',
        ...
        return (
          <RadixDialog.Portal>
            ...
            <RadixDialog.Content ref={ref} {...rest} {...getStyles()}>
              {children}
              <RadixDialog.Close {...getStyles({ part: 'closeButton' })}>
                <X className="h-4 w-4" />
