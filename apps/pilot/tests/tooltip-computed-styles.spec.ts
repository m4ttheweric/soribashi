 ...
 *   3. Arrow inherits the same bg as content (both reference --sb-tooltip-bg)
 ...
 *
 * Placement: apps/pilot/tests/ (pilot Playwright project,
...
test.describe('Tooltip — computed styles per variant', () => {
  ...
  // ── 3. Arrow inherits content bg via --sb-tooltip-bg ─────────────────────

  test('arrow fill equals content bg (both resolved from --sb-tooltip-bg)', async ({ page }) => {
    await gotoTooltipMatrix(page);
    ...
    // Both should resolve from the same --sb-tooltip-bg custom property.
