 ...
 *
 * Consolidation choices vs the host library's current tokens (see journal § 3 for full
 ...
 *   (Q2). the host library's lighter steps drift to hue 240 with 100% saturation — a
 ...
 *   anchor's hue/sat with the host library's existing lightness ramp preserved.
 ...
 *   shape, anchors, and ramp are the host library's verbatim error scale.
 * - Surface tokens collapse `card` + `popover` into a single `surface`
 *   semantic group (Q1). Both held identical values in the host library; two names with
 *   one value is duplication.
 * - Text semantics use `default`/`muted`/`subtle`/`disabled` referencing
 *   neutral scale anchors rather than the host library `text.{primary,secondary,
 ...
 * - Chart colors, `--search-*` highlight vars, `--glow-color`, and the
...
const definition = {
  ...
  tokens: {
    colors: {
      primary: {
        ...
        // as accidental). Lightnesses preserved from the host library; hue + saturation
        ...
        '500': 'hsl(221.2 83.2% 53.3%)', // anchor preserved verbatim from the host library
        ...
        // Convention from soribashi defaults / playground theme; the host library does
      ...
      neutral: {
        // the host library ramp preserved verbatim. Adds `0` for surface.default (white)
    ...
    // the host library's borderRadius extend, preserved. Note: shad's `--radius` (0.5rem)
    // is byte-identical to `radius.md` here — that duplication is captured
    // in the deprecation list (shad-* layer fully dropped).
    radius: {
      sm: '0.25rem',
      md: '0.375rem', // the host library's borderRadius.DEFAULT
      lg: '0.5rem',
      xl: '0.75rem',
      '2xl': '1rem',
      '3xl': '1.5rem', // the host library's borderRadius.2xl, renamed for ramp consistency
      full: '9999px', // not in the host library but standard for pill shapes — needed for some Button variants
    },
    // soribashi default spacing scale. the host library's five custom spacing extensions
    // (18, 88, 100, 112, 128) are DEFERRED — § 1.3.2 confirms zero utility
    // hits in the host library source.
    spacing: {
      xs: '0.25rem',
      sm: '0.5rem',
      md: '0.75rem',
    ...
    // the host library's fontSize ramp. Note the host library uses `base`; soribashi convention uses
    ...
    // (`packages/theme/src/tokens/default-tokens.ts:127`). the host library declares
    // fontSize line-heights as paired tuples in tailwind (e.g.
    // `['0.75rem', '1rem']`); each multiplier here is the host library absolute
    ...
    // the host library's boxShadow extend. `boxShadow.popover` collapsed into `lg`
  ...
  // Dark overrides per the host library's `.dark .app-scope` block. Per Q5,
  ...
  // are mode-agnostic in the host library.
  //
  // the host library's `accent.feedback` direct-semantic var has no clean home in the
  ...
  dark: {
    colors: {
      primary: {
        ...
        // light ramp (Q2). the host library's dark values held the same hue-241→217
        // drift as the light side; this rebuilds the lighter dark steps at
        // the anchor hue (217 in dark, since the host library's dark anchor itself sits
        // at 217.2). Lightnesses preserved from the host library's dark column.
        '50': 'hsl(217 91% 15%)',
        '100': 'hsl(217 91% 20%)',
        '200': 'hsl(217 91% 25%)',
        '300': 'hsl(217 91% 30%)',
        '400': 'hsl(217 91% 40%)',
        '500': 'hsl(217.2 91.2% 59.8%)', // the host library's dark `--color-primary-500`
        '600': 'hsl(217 91% 65%)',
        '700': 'hsl(217 91% 70%)',
        '800': 'hsl(217 91% 75%)',
        '900': 'hsl(217 91% 80%)',
      ...
      neutral: {
        // the host library inverts the neutral scale in dark (`50` ↔ `900` swap pattern).
  ...
  semanticTokens: {
    text: {
      ...
      // against the neutral ramp. Preserves the host library's four-step text taxonomy
    ...
    border: {
      // the host library's `--color-border-islands` (light hsl(214 32% 91%)) maps cleanly
