...
export const Select = defineGenericComponent<SelectSignature>({
  ...
  render: ({ props }: any) => {
    ...
    const parsed = useMemo(
      () => parseSelectData(props.data as SelectData<Primitive>),
      [props.data],
    );
    ...
    const [uncMulti, setUncMulti] = useState<Primitive[]>(
      Array.isArray(props.defaultValue) ? props.defaultValue : [],
    );
    const isControlled = props.value !== undefined;

    const singleValue: Primitive | null = multiple
      ? null
      : isControlled
        ? (props.value ?? null)
        : uncSingle;
    ...
    const submit = (opt: ComboboxItem<Primitive>) => {
      ...
      if (multiple) {
        ...
        if (!isControlled) {
          setUncMulti(nextValues);
        }
        props.onChange?.(nextValues, nextOptions);
        setQuery('');
      } else {
        if (!isControlled) {
          setUncSingle(opt.value);
        }
        ...
        <div style={{ position: 'relative' }}>
              ...
              placeholder={
                multiple
                  ? selectedMulti.length > 0
                    ? ''
                    : props.placeholder
                  : props.searchable && selectedSingle
                    ? selectedSingle.label
                    : props.placeholder
              }
              value={
                props.searchable ? query : !multiple && selectedSingle ? selectedSingle.label : ''
              }
              onChange={(e) => {
                if (props.searchable) {
          ...
          {opened && (
            ...
            >
              ...
              {options.length === 0 && (
                <li role="presentation" className={classes.group}>
                  Nothing found
                </li>
              )}
            </ul>
